...
const Tech: React.FC<{id:string}> = (props) => {
    return (
        <Container id={props.id}>
            ...
            <Row>
                <p className={"text-muted"}>1.给定的工艺条件下的换热计算：通过初算传热面积，计算管程和壳程流体的流动阻力复核计算是否符合工艺条件，再次复核传热面积的这样的步骤，在给定工况下，根据不同高效换热器强化特点，给出最优的设计方案，在满足换热器负荷条件下，提高经济性能，增加换热器的使用寿命。</p>
            </Row>
            <Image src={techParam.src} fluid/>
            <Row>
                <p className={"text-muted"}>2. 优化换热器的设计参数： 在工艺设计阶段或节能改造项目中，针对不同高效换热器性能（如高通量管可在小温差下沸腾的特点），可以通过选型计算，利用低品位热源替代高品位热源，大幅度减少热源消耗，从而达到冷、热能源的节能减排，对现场装置的换热经济性能显著优化。</p>
            </Row>
